...
<body>     
  ...
  <div id="grid">
    ...
    <label>Enter .mpd file: 
        <input type="text" id="filename" value="video/sample_dash.xml" />
    ...
    <div id="col2">
      <!-- Video element -->
      <video id="myVideo" autoplay="autoplay" controls>No video available</video>
            <div id="description">
  ...
  <script>
      ...
      if (bufferUpdated == true) {
        if (index < segments.length) {
          ...
          if ((videoElement.currentTime - lastTime) >= segCheck) {
			...
			//indexSegment(segments[index].getAttribute("indexRange").toString(), file);
	...
	function indexSegment(range, url)
	{
		console.log('AAA indexSegment = ', range, '  ', url);
		var xhr = new XMLHttpRequest();
		xhr.open('GET', url);
        xhr.setRequestHeader("Range", "bytes=" + range);
        xhr.send();
		...
		xhr.addEventListener("readystatechange", function () {
		if (xhr.readyState == xhr.DONE) {
			console.log('AAA indexSegment ', xhr.response);
		  }
	    }, false);
	}
