...
<body>
	...
	function initVideo(url) {
		...
		var urlVideo = initializationVideo;
		xhrV.open('GET', urlVideo);
		segCheckVideo = durationsVideo[0] * .50;
		xhrV.send();
		xhrV.responseType = 'arraybuffer';
		xhrV.addEventListener("readystatechange", function () {
			if (xhrV.readyState == xhrV.DONE) { // && xhrV.status == 200
				videoSource.appendBuffer(new Uint8Array(xhrV.response));
				videoSource.addEventListener("update",updateFunctVideo);
			}
		}, false);
		
		// Init audio request
		var urlAudio = initializationAudio;
		var xhrA = new XMLHttpRequest();
		xhrA.open('GET', urlAudio);
		segCheckAudio = durationsAudio[0] * .50;
		xhrA.send();
		xhrA.responseType = 'arraybuffer';
		xhrA.addEventListener("readystatechange", function () {
			if (xhrA.readyState == xhrA.DONE) { // && xhrA.status == 200
				audioSource.appendBuffer(new Uint8Array(xhrA.response));
	...
	function playSegment(url, ind, isVideo) {
		if (url) { // Make sure we've got incoming params
			...
			xhrSeg.open('GET', url);
			xhrSeg.responseType = 'arraybuffer';
			xhrSeg.send();
			xhrSeg.addEventListener("readystatechange", function () {
				if (xhrSeg.readyState == xhrSeg.DONE && xhrSeg.status == 200) {
					...
					try {
						targetSource.appendBuffer(new Uint8Array(xhrSeg.response));
					} catch(e) {
